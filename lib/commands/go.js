...
var go = module.exports = function(group, alias, callback) {
  ...
  var writeTarget = function(result, output) {
        fs.writeFile(app.config.get('tmpDirFile'), result.entry.path, function(e) {
          if (e) {
            return callback(util.format('The path %s could not be written to: %s', app.config.get('tmpDirFile').yellow.bold, e.red.bold));
          } else {
